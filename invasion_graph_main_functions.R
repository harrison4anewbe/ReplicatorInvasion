...
# Comment from meeting about replicator eqns:
# x'=x*(P%*%x-t(x)%*%P%*%x))
# solve using temp=solve(a=P,b=rep(1,k))
# xstar=temp/sum(temp)
LV.IS=function(A,b,tolerance=1e-14){
  ...
  for(i in 1:k){ #loop for the number of species supported by the equilibrium. Note: for replicator this will start with 2 instead of 1
    ...
    for(j in 1:k2){ # loop through the configurations
      ...
      xtemp=solve(a = A[I,I],b=-b[I]) # solve for the equilibrium restricted to the species in I
  ...
  for(i in 1:length(C)){
    IS[i,]=A%*%C[[i]]+b # i-th row corresponds to all the per-capita growth rates at the i-th feasible equilibrium
  }
